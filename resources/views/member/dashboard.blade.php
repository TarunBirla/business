...
@section('content')
<!-- Welcome Banner -->
<div class="rounded-2xl text-white p-8 mb-8 shadow-md" style="background-color: var(--btn-primary-bg);">
...
<div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
    <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div class="text-xs font-bold text-black uppercase">My Communities</div>
        <div class="text-3xl font-bold text-black mt-2">{{ $myGroups->count() }}</div>
    </div>

    <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div class="text-xs font-bold text-black uppercase">My Connections</div>
        <div class="text-3xl font-bold text-sky-600 mt-2">{{ $myConnectionsCount }}</div>
    </div>

    <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div class="text-xs font-bold text-black uppercase">Pending Requests</div>
        <div class="text-3xl font-bold text-amber-500 mt-2">{{ $pendingConnectionsCount }}</div>
    </div>

    <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div class="text-xs font-bold text-black uppercase">Upcoming Events</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <div class="lg:col-span-2 space-y-8">
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <div class="flex items-center justify-between mb-6">
                <h3 class="text-xl font-bold text-black">My Joined Communities</h3>
                ...
                @forelse($myGroups as $group)
                    <div class="p-4 rounded-xl border border-slate-100 bg-slate-50 flex items-center justify-between">
                        <div class="flex items-center space-x-4">
                            ...
                            <div>
                                <h4 class="font-bold text-black text-base">{{ $group->name }}</h4>
                                <p class="text-xs text-black">Joined {{ $group->pivot->joined_at ? \Carbon\Carbon::parse($group->pivot->joined_at)->diffForHumans() : 'Recently' }}</p>
                            </div>
                        </div>
                        <a href="{{ route('groups.show', $group->slug) }}" class="px-4 py-2 bg-white border border-slate-200 hover:bg-slate-100 text-black font-bold text-xs rounded-lg transition">
                            View Page
                        </a>
                    </div>
                @empty
                    <p class="text-sm text-black">You have not joined any community yet.</p>
                @endforelse
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <h3 class="text-xl font-bold text-black mb-6">Latest Community Announcements</h3>
            <div class="space-y-4">
                @forelse($latestNotices as $notice)
                    <div class="p-4 rounded-xl border border-sky-100 bg-sky-50/40">
                        <div class="flex items-center justify-between">
                            ...
                            <span class="px-2 py-0.5 rounded text-[10px] font-bold uppercase {{ $notice->priority === 'urgent' ? 'bg-rose-100 text-rose-800' : 'bg-slate-200 text-black' }}">
                                {{ $notice->priority }}
                            </span>
                        </div>
                        <h4 class="font-bold text-black mt-2 text-base">{{ $notice->title }}</h4>
                        <p class="text-xs text-black mt-1 leading-relaxed">{{ $notice->content }}</p>
                    </div>
                @empty
                    <p class="text-sm text-black">No active notices.</p>
                @endforelse
            </div>
        </div>
    ...
    <div class="space-y-8">
        <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <h3 class="text-xl font-bold text-black mb-6">My Registered Events</h3>
            <div class="space-y-4">
                @forelse($upcomingEvents as $reg)
                    <div class="p-4 rounded-xl border border-slate-200 bg-slate-50">
                        <h4 class="font-bold text-black text-sm">{{ $reg->event->title }}</h4>
                        ...
                        <p class="text-xs text-black mt-0.5"><i class="fa-solid fa-location-dot text-rose-500 mr-1"></i>{{ $reg->event->venue ?? 'Online' }}</p>
                    </div>
                @empty
                    <p class="text-sm text-black">No event registrations found.</p>
                @endforelse
            </div>
        </div>
    </div>
