...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-black">Group Members ({{ $group->name }})</h1>
            <p class="text-sm text-black mt-1">Manage registrations, invite new members, and export CSV reports.</p>
        </div>
        <div class="flex space-x-3">
            <a href="{{ route('group_admin.members.export_csv', $group->id) }}" class="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-black font-bold text-xs rounded-xl transition">
                📥 Export CSV
            </a>
            ...
            <tbody class="divide-y divide-slate-100 text-sm">
                @foreach($members as $m)
                    <tr class="hover:bg-slate-50">
                        <td class="p-4 font-bold text-black">{{ $m->name }}</td>
                        <td class="p-4 text-black">{{ $m->email }}</td>
                        <td class="p-4 text-black">{{ $m->phone ?? 'N/A' }}</td>
                        <td class="p-4 text-sky-600 font-semibold">{{ $m->profession ?? 'N/A' }}</td>
                        <td class="p-4 text-black">{{ $m->city ?? 'N/A' }}</td>
                        <td class="p-4">
                        ...
                        <td class="p-4 text-xs text-black">{{ \Carbon\Carbon::parse($m->pivot->joined_at)->format('d M Y') }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
