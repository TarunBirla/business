...
<div class="space-y-8">
    ...
    <div class="flex items-center justify-between bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
        <div>
            ...
            <p class="text-xs text-black mt-1">Customize color tokens with real-time live preview for {{ $group->name }}.</p>
        </div>
        <a href="{{ route('group_admin.themes.index', $group->id) }}" class="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-xs rounded-xl transition">
            &larr; Back to Community Themes
        </a>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <!-- Form Left Column -->
        <form action="{{ $isEdit ? route('group_admin.themes.update', [$group->id, $theme->id]) : route('group_admin.themes.store', $group->id) }}" method="POST" class="lg:col-span-2 space-y-6">
            @csrf
            @if($isEdit)
                @method('PUT')
            @endif
            ...
            <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="text" name="name" value="{{ old('name', $theme->name) }}" required placeholder="e.g. Gujarati Teal Theme" class="w-full px-4 py-3 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-sky-500">
                    </div>
                    <div>
            ...
            <div class="pt-4">
                <button type="submit" class="w-full py-4 bg-teal-600 hover:bg-teal-700 text-white font-bold text-base rounded-2xl shadow-xl transition">
                    {{ $isEdit ? 'Save Theme Changes' : 'Create & Apply Theme' }}
                </button>
            </div>
        </form>
        ...
        <div class="sticky top-24 bg-white rounded-2xl border border-slate-200 shadow-xl overflow-hidden">
            <div class="p-4 bg-slate-900 text-white flex items-center justify-between">
                <span class="text-xs font-bold uppercase tracking-wider flex items-center">
                    <i class="fa-solid fa-eye text-teal-400 mr-2"></i>Live Theme Preview
                </span>
            ...
            <div id="previewCanvas" class="p-6 space-y-6 text-sm overflow-y-auto max-h-[750px] transition-colors duration-200">
                ...
                <div class="p-3 rounded-xl shadow-sm flex items-center justify-between border" style="background-color: var(--navbar-bg); border-color: var(--border-color);">
                    <span class="font-bold text-sm" style="color: var(--navbar-text);">
                        <i class="fa-solid fa-layer-group mr-1" style="color: var(--icon-color);"></i>{{ $group->name }}
                    </span>
                ...
                <div class="p-5 rounded-2xl border shadow-sm space-y-3" style="background-color: var(--card-bg); border-color: var(--card-border);">
                    ...
                    <h4 class="font-bold text-base" style="color: var(--card-title);">{{ $group->name }} Business Meetup</h4>
                    <p class="text-xs leading-relaxed" style="color: var(--card-body);">
                        Connecting professionals and business leaders in your community.
                    </p>
