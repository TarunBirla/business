...
<div class="space-y-8">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <h1 class="text-3xl font-bold text-black mt-1">{{ $activeGroup->name }}</h1>
        </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
        <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <div class="text-xs font-bold text-black uppercase">Total Members</div>
            <div class="text-3xl font-bold text-black mt-2">{{ number_format($memberCount) }}</div>
        </div>
        <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <div class="text-xs font-bold text-black uppercase">New (Last 30 Days)</div>
            <div class="text-3xl font-bold text-sky-600 mt-2">+{{ number_format($newMembersCount) }}</div>
        </div>
        <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <div class="text-xs font-bold text-black uppercase">Events</div>
            <div class="text-3xl font-bold text-emerald-600 mt-2">{{ number_format($eventsCount) }}</div>
        </div>
        <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <div class="text-xs font-bold text-black uppercase">Notices</div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <div class="flex items-center justify-between mb-4">
                <h3 class="font-bold text-black text-lg">Recent Registrations</h3>
            ...
            <div class="space-y-3">
                @foreach($recentMembers as $m)
                    <div class="p-3 bg-slate-50 rounded-xl flex items-center justify-between">
                        <div>
                            <div class="font-bold text-black text-sm">{{ $m->name }}</div>
                            <div class="text-xs text-black">{{ $m->email }} &bull; {{ $m->profession ?? 'Member' }}</div>
                        </div>
        ...
        <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <div class="flex items-center justify-between mb-4">
                <h3 class="font-bold text-black text-lg">Upcoming Events</h3>
            ...
            <div class="space-y-3">
                @foreach($upcomingEvents as $evt)
                    <div class="p-3 bg-slate-50 rounded-xl flex items-center justify-between">
                        <div>
                            <div class="font-bold text-black text-sm">{{ $evt->title }}</div>
