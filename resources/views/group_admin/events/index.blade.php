    ...
    @if(isset($assignedGroups) && $assignedGroups->count() > 1)
        ...
        <div class="flex items-center space-x-2 border-b border-slate-200 pb-3 overflow-x-auto">
            ...
            @foreach($assignedGroups as $ag)
                @php
                    $eCount = $ag->events()->count();
                @endphp
                <a href="{{ route('group_admin.events.index', $ag->id) }}" class="px-4 py-2 rounded-xl text-xs font-bold transition shrink-0 flex items-center space-x-2 {{ $ag->id === $group->id ? 'bg-sky-600 text-white shadow-2xs' : 'bg-white text-slate-700 hover:bg-slate-100 border border-slate-200' }}">
                    <span>{{ $ag->name }}</span>
