...
<div class="max-w-3xl mx-auto space-y-8">
    <div class="flex items-center justify-between border-b pb-6 border-slate-200">
        ...
        <a href="{{ route('group_admin.projects.index', ['tab' => 'my_projects']) }}" class="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 font-semibold text-sm rounded-xl transition">
            &larr; Back to Projects
        </a>
    </div>

    <form method="POST" action="{{ $project->exists ? route('group_admin.projects.update', $project->id) : route('group_admin.projects.store') }}" enctype="multipart/form-data" class="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm space-y-6">
        @csrf
        @if($project->exists)
            @method('PUT')
        @endif
        ...
        <div class="pt-4 flex items-center justify-end space-x-4 border-t border-slate-100">
            <a href="{{ route('group_admin.projects.index', ['tab' => 'my_projects']) }}" class="px-6 py-3 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold text-sm rounded-xl transition">
                Cancel
            </a>
