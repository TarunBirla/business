...
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl font-bold text-slate-900">Group Notices & Announcements</h1>
            <p class="text-sm text-slate-600 mt-1">Publish important notices to members of {{ $group->name }}.</p>
        </div>
    ...
    <div class="space-y-4">
        @foreach($notices as $notice)
            <div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
                <div class="flex items-center justify-between">
                    <span class="px-2.5 py-1 rounded text-xs font-bold uppercase {{ $notice->priority === 'urgent' ? 'bg-rose-100 text-rose-800' : 'bg-slate-100 text-slate-800' }}">
                        Priority: {{ $notice->priority }}
                    </span>
                    <span class="text-xs text-slate-400 font-medium">Published {{ $notice->published_at->format('d M Y') }}</span>
                </div>
                <h3 class="font-bold text-slate-900 text-xl mt-3">{{ $notice->title }}</h3>
